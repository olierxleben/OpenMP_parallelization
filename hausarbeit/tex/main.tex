...
Beispiele finden für Sections finden sich im Anhang, Listing \ref{sectionsExample}, bzw. Listing \ref{taskExample} für Tasks. 
...
Das \textbf{Tasks}-Konzept ist die größte Erneuerung für OpenMP mit dem Release der Version 3.0. Die Definition eines Tasks akann an beliebiger Stelle passieren. Im Unterschied zu Sections kann ein Task auch durch einen beliebigen Thread definiert werden. Es gibt drei Möglichkeiten wie ein Thread einen Task behandeln kann, sobald dieser auf eine Task-Direktive trifft:
\begin{itemize}
...
\lstinputlisting[caption=Bash-Skript zur Erfassung der Laufzeiten, label= ScriptMeasurement]{../src/measurement/measure}
...
\lstinputlisting[caption= Umgebungsvariablen und Bibliotheksfunktionen, label=envvarsLibfuncs]{../src/envvars_libfuncs/envvars_libfuncs.c}

\subsection{Parallele Schleifen - Hello OMP Parallel}

\lstinputlisting[caption= pragma omp parallel - Beispiel, label=parallelExample, language=Bash]{../src/loops/parallel.c}
...
\lstinputlisting[caption= pragma omp parallel for - Beispiel, label=parallelForExample]{../src/loops/parallelfor.c}
In diesem Beispiel wird der zu parallelisierende Bereich eine Variable \textit{tid}, die threadId, übergeben. Jeder Thread erhält diese Variable, die nicht gemeinsam genutzt werden kann (private). Mittels \texttt{\#pragma omp for} wird dem Compiler mitgeteilt das die darauffolgende Schleife parallel durch das team of threads  im parallelen Bereich ausgeführt werden soll.

\subsection{Section-Beispiel }
\lstinputlisting[caption= Section-Beispiel, label=sectionsExample]{../src/sections/sections.c}
...
\lstinputlisting[caption= Task-Beispiel: Fibonacci, label=taskExample]{../src/tasks/fib.c}
...
Im Quelltext des Beispiels soll die Fibunacci-Zahlenfolge für n = 7 berechnet werden. Für n > 2 kann nun die vorherige Zahl (n -1) und die vorvorherige Zahl (n - 2) berechnet werden. Das Ergbnis ist die Summe aus den beiden Teilergebnissen i und j, welches zurückgegeben wird. Zu beachten ist, das \textit{i} und \textit{j} im gesamten Adressraum zu Verfügung stehen und jeder Task sein eigenes \textit{n} verwendet. Jede für n = 7 erzeugt der Algorithmus nun jeweils zwei weitere Berechnungen bis n <= 2 ist, da für 1 und 2 keine Berechnungen erforderlich sind und damit auch das Abbruckriterium der Rekursion darstellen.  
